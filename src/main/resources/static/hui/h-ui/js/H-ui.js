...
(function($) {
	'use strict';
	var name = 'colorPicker'; // modal name
	var TEAMPLATE = '<div class="colorpicker"><button type="button" class="hui-btn hui-dropdown-toggle" data-toggle="dropdown"><span class="cp-title"></span><i class="ic"></i></button><ul class="hui-dropdown-menu clearfix"></ul></div>';
	var LANG = {
		zh_cn: {
			errorTip: "不是有效的颜色值"
		},
	...
	ColorPicker.prototype.init = function() {
		...
		this.$menu = this.$picker.find('.hui-dropdown-menu').toggleClass('pull-right', options.pullMenuRight);
...
/* =======================================================================
 * jQuery.Huihover.js v2.0 hover效果
 * http://www.h-ui.net/
 * Created & Modified by guojunhui
	...
	$.fn.Huitag = function(options) {
		...
		this.each(function(){
			...
			'<div class="hui-tag-wrapper">'+
				'<div class="hui-tag-editor clearfix"></div>'+
				'<div class="hui-tag-input-wrapper">'+
					'<input type="text" class="hui-input-text hui-tag-input" maxlength="'+options.maxlength+'" value="">'+
				'</div>'+
				'<div class="hui-tag-list">'+
					'<div class="hui-tag-notag" style="display:none">暂无常用标签</div>'+
					'<div class="hui-tag-has"></div>'+
				'</div>'+
				'<input type="hidden" class="hui-tag-val" name="" value="'+options.value+'">'+
			'</div>';
			that.append(str);
			var wrapper = that.find(".hui-tag-wrapper");
			...
			if(options.tagsDefault){
				...
				has.find("span").on('click',function(){
					...
					if(dataStrArr.toString().indexOf(taghasV)>-1){
						...
						editor.append('<span class="hui-tag-token">'+taghasV+'</span>');
						gettagval(this);
						$(this).remove();
					}
				});
			...
			function gettagval(obj) {
				var str = "";
				var token = that.find(".hui-tag-token");
				if (token.length < 1) {
					input.val("");
					return false;
				}
			...
			input.on("change",function(){
				...
				var v = v2+','+v1;
				if(v!=''){
					var str='<i class="hui-tag-icon hui-iconfont">&#xe64b;</i>';
					var result = unique(v, 1);
					if(result.length>0){
						for(var j=0;j<result.length;j++){
							str+='<span class="hui-tag-token">'+result[j]+'</span>';
						}
						val.val(result);
						editor.html(str);
						input.val("").blur();
			...
			$(document).on("click",".hui-tag-token",function(){
				$(this).remove();
				var str ="";
				if(that.find(".hui-tag-token").length<1){
					val.val("");
					return false;
				}else{
					for(var i = 0;i< that.find(".hui-tag-token").length;i++){
						str += that.find(".hui-tag-token").eq(i).text() + ",";
					}
...
!function($) {
	$.fn.Huitextarealength = function(options){
		...
		this.each(function(){			
			...
			var v = that.val();
			var l = v.length;
			var str = '<p class="hui-textarea-numberbar"><em class="hui-textarea-length">'+l+'</em>/'+options.maxlength+'</p>';
			...
			that.on("keyup",function(){
				...
				that.parent().find(".hui-textarea-length").text(v.length);
			});		
				...
				function() {
					...
					timer = setTimeout(function () {
						...
						if (bigImg == '') {
							...
							'<div id="preview-wrapper" style="position: absolute;z-index:999;width:'+options.bigImgWidth+'px;height:auto;top:' + imgT + 'px;right:' + tooltipRight + ';left:' + tooltipLeft + '">'+
							...
							image.onload = function() {
								$('#preview-wrapper').find("img").attr("src",bigImg).css("width",options.bigImgWidth);
							};
						}
					},500);
				},
...
!function($) {
  $.fn.Huimarquee = function(options) {
    ...
    this.each(function() {
      var that = $(this);
      if (that.find(".hui-marquee-content").height() > options.height) {
        var pause = false;
        var scrollT;
        ...
        that.scrollTop(0);
        var start = function() {
          scrollT = setInterval(scrolling, options.speed);
          if (!pause) {
            that.scrollTop(that.scrollTop() + 2);
          }
        }
        var scrolling = function() {
          if (that.scrollTop() % options.height != 0) {
...
!function($) {
	...
	function(e) {
		var $btn = $(e.target);
		if (!$btn.hasClass('btn'));
		$btn = $btn.closest('.hui-btn');
		Plugin.call($btn, 'toggle');
		e.preventDefault();
	}).on('focus.bs.button.data-api blur.bs.button.data-api', '[data-toggle^="button"]',
	function(e) {
		$(e.target).closest('.hui-btn').toggleClass('focus', e.type == 'focus');
	})
} (jQuery);
/* =======================================================================
...
!function($) {
  ...
  var Modal = function(element, options) {
    ...
    if (this.options.remote) {
      this.$element.find('.hui-modal-content').load(this.options.remote, $.proxy(function() {
  ...
  Modal.prototype.show = function(_relatedTarget) {
    ...
    this.backdrop(function() {
      ...
      transition ? that.$element.find('.hui-modal-dialog') // wait for modal to slide in
      .one('bsTransitionEnd',
      function() {
  ...
  Modal.prototype.hideModal = function() {
    ...
    this.$element.hide();
    this.backdrop(function() {
      that.$body.removeClass('hui-modal-open');
      that.resetScrollbar();
  ...
  Modal.prototype.backdrop = function(callback) {
    ...
    if (this.isShown && this.options.backdrop) {
      ...
      this.$backdrop = $('<div class="hui-modal-backdrop ' + animate + '" />').prependTo(this.$element).on('click.dismiss.bs.modal', $.proxy(function(e) {
  ...
  Modal.prototype.measureScrollbar = function() { // thx walsh
    ...
    scrollDiv.className = 'hui-modal-scrollbar-measure';
...
!function($) {
	'use strict';
	var backdrop = '.hui-dropdown-backdrop';
	...
	Dropdown.prototype.toggle = function(e) {
		...
		clearMenus();
		if (!isActive) {
			if ('ontouchstart' in document.documentElement && !$parent.closest('.hui-navbar-nav').length) {
				// if mobile we use a backdrop because click events don't delegate
				$(document.createElement('div')).addClass('hui-dropdown-backdrop').insertAfter($(this)).on('click', clearMenus);
			}
			var relatedTarget = {
				relatedTarget: this
			}
	...
	Dropdown.prototype.keydown = function(e) {
		...
		var $items = $parent.find('.hui-dropdown-menu' + desc);
	...
	$(document).on('click.bs.dropdown.data-api', clearMenus).on('click.bs.dropdown.data-api', '.hui-dropdown form',
	function(e) {
		e.stopPropagation()
	}).on('click.bs.dropdown.data-api', toggle, Dropdown.prototype.toggle).on('keydown.bs.dropdown.data-api', toggle, Dropdown.prototype.keydown).on('keydown.bs.dropdown.data-api', '.hui-dropdown-menu', Dropdown.prototype.keydown);
} (window.jQuery);
...
! (function(factory) {
	...
	var Datetimepicker = function(element, options) {
		...
		if (this.isInline) {
			...
			this.picker.addClass('datetimepicker-dropdown-' + this.pickerPosition + ' hui-dropdown-menu');
		}
		if (this.isRTL) {
	...
	*/
	...
	DPGlobal.template = '<div class="datetimepicker">' + '<div class="datetimepicker-minutes">' + '<table class=" table-condensed">' + DPGlobal.headTemplate + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-hours">' + '<table class=" table-condensed">' + DPGlobal.headTemplate + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-days">' + '<table class=" table-condensed">' + DPGlobal.headTemplate + '<tbody></tbody>' + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-months">' + '<table class="hui-table-condensed">' + DPGlobal.headTemplate + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-years">' + '<table class="hui-table-condensed">' + DPGlobal.headTemplate + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '</div>';
	DPGlobal.templateV3 = '<div class="datetimepicker">' + '<div class="datetimepicker-minutes">' + '<table class=" table-condensed">' + DPGlobal.headTemplateV3 + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-hours">' + '<table class=" table-condensed">' + DPGlobal.headTemplateV3 + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-days">' + '<table class=" table-condensed">' + DPGlobal.headTemplateV3 + '<tbody></tbody>' + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-months">' + '<table class="hui-table-condensed">' + DPGlobal.headTemplateV3 + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '<div class="datetimepicker-years">' + '<table class="hui-table-condensed">' + DPGlobal.headTemplateV3 + DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>' + '</div>' + '</div>';
