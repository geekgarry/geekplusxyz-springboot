...
import com.maike.common.entity.FontText;
...
//    private static QiniuUtil qiniuUtil = new QiniuUtil();
	...
	* @date 2018年9月1日上午8:49:21
	 ...
	 * @Title: buildShareImg
	 * @Description: void
	 * @param logoTxt logo内容 中微直供|分销商名称|负责人名称
	 * @param productTitle 商品标题
	 * @param productPrice 商品价格
	 * @param codeUrl 分享二维码链接地址
	 * @param logoImgPath logo分销商图片地址
	 * @param productPath 商品图片地址
	 * @author WeiZheng
	 * @date 2018年8月31日下午3:38:26
	 */
	public static String buildShareImg(String logoTxt,String productTitle,String productPrice,String codeUrl,String logoImgPath,String productPath) {
		String shareImgName = getShareImgName(logoTxt, productTitle, productPrice, codeUrl, logoImgPath, productPath)+".png";
		/**
		 * 判断该商品是否存在
		 */
		String shareImgPath = RELATIVE_PARENT_PATH+"share/"+shareImgName;
		File shareImg = new File(shareImgPath);
		if(shareImg.exists()){
			logger.info(" {} 图片已经生成过，不用生成。",shareImgName);
			return  "http://cdn.juheyinhang.com/"+shareImgName;
		}
		long startTime = System.currentTimeMillis();
		BufferedImage destImg = null;
		try {
			destImg = getBufferedImage(RELATIVE_TEMPLATE_PATH);
		} catch (IOException e) {
			logger.error(e.getMessage());
		}
		if (null == destImg) {
			logger.info("destImg is null");
			return "";
		}
		/**
		 * 添加文字信息
		 */
		/**
		 * String productPath2 = "http://cdn.juheyinhang.com/TB2PsTZgUQIL1JjSZFhXXaDZFXa-1714128138.jpg";
		 */
		appendTxtInfo(destImg, logoTxt, productTitle, productPrice);
		/**
		 * 压缩logo图片 58px 添加logo
		 */
		appendLogoImg(destImg, logoImgPath);
		/**
		 * 商品区 需要进行裁剪 居中剪裁 1:1 0,314 809,1125 810px 先缩放 在裁剪
		 */
		appendProductImg(destImg, productPath);
		/**
		 * 二维码区域 左侧 35,1142 259,1367 225 生成二维码 添加到图片中
		 */
		appendQRCode(destImg, codeUrl);
		/**
		 * 保存
		 */
		saveImage(destImg, RELATIVE_PARENT_PATH+"share/", shareImgName, "png");
		/**
		 * 上传到七牛云
		 */
		shareImgName  = "123";//qiniuUtil.uploadImg(new FileInputStream(new File(shareImgPath)), shareImgName);
		logger.info("图片处理消耗: {} 毫秒", System.currentTimeMillis() - startTime);
		return shareImgName;
	}

	public static void main(String[] args) {
		String logoTxt = "中微直供|宿迁学院站|任斌";
		String productTitle = "Xiaomi Pro 全金属版 8G+256G";
		String productPrice = "4999元";
		String codeUrl = "https://user.qzone.qq.com/710007248/infocenter";
		String logoImgPath = "http://cdn.juheyinhang.com/f8de2622-401a-4714-8d9e-bf2f90757c481535335654276.jpg";
		//String productPath = "http://cdn.juheyinhang.com/product.png";
		String productPath = "http://cdn.juheyinhang.com/TB2PsTZgUQIL1JjSZFhXXaDZFXa-1714128138.jpg";
		String shareImgName = buildShareImg(logoTxt, productTitle, productPrice, codeUrl, logoImgPath, productPath);
		logger.info("七牛云分享图片地址: {}",shareImgName);
	}
}
