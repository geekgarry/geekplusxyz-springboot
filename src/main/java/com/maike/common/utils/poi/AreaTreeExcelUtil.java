...
public class AreaTreeExcelUtil {
  ...
  public static List<?> getContent(Sheet sheet) {
    ...
    System.out.println("数据字段列表"+ JsonUtil.objectToJsonStr(titles));
...
//    fbfMap.put("fbf",fbf);
    ...
    for (int i = 5; i<=rowCount; i++){
      ...
      int cbfRowIndex = 0;
      for(int j=3;j<=8;j++){
        if(StringUtils.isNotEmpty(getCellValue(cbfRow,j).toString())){
          cbfRowIndex=i;
      ...
      for(int x=cbf98index;x<=columnCount;x++){
        if(StringUtils.isNotEmpty(getCellValue(cbfRow,x).toString())){
    ...
    for(int y=0;y<cbfList.size();y++){
      ...
      if(y==cbfList.size()-1){
        ...
        for(int m=cbfRowNum-5;m<=allRowCount-5;m++){
          ...
          if(m==(dkRowIndex-5)){
            if(StringUtils.isNotEmpty(dkxxList.get(m).get("dklb").toString())) {
          ...
          if(m==(familyRowIndex-5)){
            if(StringUtils.isNotEmpty(familyList.get(m).get("cyxm").toString())) {
